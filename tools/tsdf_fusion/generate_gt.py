...
def parse_args():
    ...
    parser.add_argument("--data_path", metavar="DIR",
                        help="path to raw dataset", default='/home/qzp/3Ddata/RawData/indoor/')#############################need to reviewd
    parser.add_argument("--save_name", metavar="DIR",
                        help="file name", default='all_tsdf')
    parser.add_argument('--test', action='store_true',
                        help='prepare the test set')
    parser.add_argument('--max_depth', default=3., type=float,#############################need to reviewd 120
                        help='mask out large depth values since they are noisy')
    parser.add_argument('--min_depth', default=0., type=float,#############################need to reviewd
                        help='mask out small depth values since they are noisy')
    parser.add_argument('--num_layers', default=4, type=int)
    parser.add_argument('--margin', default=4, type=int)#########################################need to reviewd 5
    parser.add_argument('--voxel_size', default=0.04, type=float) #########################################need to reviewd 1.0

    parser.add_argument('--window_size', default=9, type=int) #########################################need to reviewd 9
    parser.add_argument('--min_angle', default=15, type=float)##orgin:15  0
    parser.add_argument('--min_distance', default=0.1, type=float)##orgin:0.1 0
    parser.add_argument('--datatype', default="indoor") #########################################need to reviewd mountain
...
def save_tsdf_full(args, scene_path, cam_intr, depth_list, cam_pose_list, color_list, save_mesh=False):
    ...
    for id in depth_list.keys():
        if id % 100 == 0:
            #print(tsdf_vol_list[0]._tsdf_vol_cpu)
            print("{}: Fusing frame {}/{}".format(scene_path, str(id), str(n_imgs)))
        #print(tsdf_vol_list[0]._tsdf_vol_cpu.sum().sum())
        depth_im = depth_list[id]
        cam_pose = cam_pose_list[id]
        if len(color_list) == 0:
            print("color_image dose not exist")
            color_image = None
        else:
            color_image = color_list[id]
            #print(np.size(color_image,0),np.size(color_image,1),np.size(color_image,2))
    ...
    for l in range(args.num_layers):
        ...
        meshwrite(os.path.join(args.save_path, scene_path, 'mesh_layer{}_self.ply'.format(str(l))), verts, faces, norms,colors)
...
def save_fragment_pkl(args, scene, cam_intr, depth_list, cam_pose_list):
    ...
    if args.datatype == "mountain" or args.datatype == "indoor":

        id = list(depth_list.keys())
        print(id)
        for i in range(0,len(id),3):
            if i + args.window_size > len(id)-1:
                break
            depth_im = depth_list[id[i]]
            cam_pose = cam_pose_list[id[i]]
            ...
            vol_bnds[:, 0] = np.minimum(vol_bnds[:, 0], np.amin(view_frust_pts, axis=1))
            vol_bnds[:, 1] = np.maximum(vol_bnds[:, 1], np.amax(view_frust_pts, axis=1))
            for j in range(args.window_size-1):
                ids.append(id[i+j+1])
                depth_im = depth_list[id[i+j+1]]
                cam_pose = cam_pose_list[id[i+j+1]]
            # Compute camera view frustum and extend convex hull
                view_frust_pts = get_view_frustum(depth_im, cam_intr, cam_pose)
                vol_bnds[:, 0] = np.minimum(vol_bnds[:, 0], np.amin(view_frust_pts, axis=1))
                vol_bnds[:, 1] = np.maximum(vol_bnds[:, 1], np.amax(view_frust_pts, axis=1))
...
def process_with_single_worker(args, scannet_files):
    for scene in tqdm(scannet_files):
        ...
        if args.dataset == 'scannet':
            if args.datatype=="mountain":
                n_imgs = len(os.listdir(os.path.join(args.data_path, scene, 'color')))
                intrinsic_dir = os.path.join(args.data_path, scene, 'intrinsic', 'intrinsic.txt')
                cam_intr = np.loadtxt(intrinsic_dir, delimiter=',')[:3, :3]
                dataset = ScanNetDataset(n_imgs, scene, args.data_path, args.max_depth, args.min_depth)
            if args.datatype == "indoor":
                n_imgs = len(os.listdir(os.path.join(args.data_path, scene, 'color')))
                intrinsic_dir = os.path.join(args.data_path, scene, 'intrinsics_depth.txt')
                cam_intr = np.loadtxt(intrinsic_dir)[:3, :3]
                dataset = ScanNetDataset(n_imgs, scene, args.data_path, args.max_depth, args.min_depth,datatype="indoor")
...
if __name__ == "__main__":
    print('需要主义： 由于blender数据制作的bug，这里社区了地0张图像！！！！！！！！在dataset哪里做了修改')
    # print('怀疑数据集有问题 先不去掉第一个图了')
