...
class NeuConNet(nn.Module):
    ...
    def __init__(self, cfg, IsNeuralRecon):
        super(NeuConNet, self).__init__()
        self.IsNeuralRecon = IsNeuralRecon
        self.back_project = nn.ModuleList()
        self.back_project.append(Back_project(80))
        self.back_project.append(Back_project(40))
        self.back_project.append(Back_project(24))
        ...
        if self.cfg.FUSION.FUSION_ON:
            # GRU Fusion
            self.gru_fusion = GRUFusion(cfg, channels, IsNeuralRecon =self.IsNeuralRecon)
        # sparse conv
        self.sp_convs = nn.ModuleList()
        # MLPs that predict tsdf and occupancy.
        self.tsdf_preds = nn.ModuleList()
        self.occ_preds = nn.ModuleList()
        self.tsdf_dis = nn.ModuleList()
        self.occ_dis = nn.ModuleList()
    ...
    def forward(self, features, inputs, outputs):
        ...
        for i in range(self.cfg.N_LAYER):
            loss_dict.update({f'tsdf_occ_loss_{i}': 0.0})
        # ----coarse to fine----
        for i in range(self.cfg.N_LAYER):
            ...
            if i == 0:
                # ----generate new coords----
                coords = generate_grid(self.cfg.N_VOX, interval)[0]##N_VOX:[96, 96, 96] coords:1 3 n-points
                up_coords = []
                for b in range(bs):
                    up_coords.append(torch.cat([torch.ones(1, coords.shape[-1]).to(coords.device) * b, coords]))
                up_coords = torch.cat(up_coords, dim=1).permute(1, 0).contiguous() ##b*n-points 4
                # print('up_coords0:',up_coords.shape)
            else:
                # ----upsample coords----
                up_feat, up_coords = self.upsample(pre_feat, pre_coords, interval)
            # print('up_coords:',up_coords.shape)
            # ----back project----
            feats = torch.stack([feat[scale] for feat in features]) ##n_view b c h

            KRcam = inputs['proj_matrices'][:, :, scale].permute(1, 0, 2, 3).contiguous()
            volume, count = self.back_project[i].back_project(up_coords, inputs['vol_origin_partial'], self.cfg.VOXEL_SIZE, feats,KRcam, self.IsNeuralRecon)
            ...
            # ----gru fusion----
            if self.cfg.FUSION.FUSION_ON:
                up_coords, feat, tsdf_target, occ_target = self.gru_fusion(up_coords, feat, inputs, i)##qizipeng:transformer fusion
            ...
            if tsdf_target is not None:
                # print(torch.sum(tsdf),torch.sum(tsdf_target),torch.sum(occ),torch.sum(occ_target))
            ...
            tsdf_target = tsdf_target[occupancy]
            # print(torch.max(pre_tsdf), torch.min(pre_tsdf))
            # print(torch.max(pre_occ), torch.min(pre_occ))
            # print(torch.max(tsdf_target), torch.min(tsdf_target))

            pre_feat = torch.cat([pre_feat, pre_tsdf, pre_occ], dim=1)

            if i == self.cfg.N_LAYER - 1:
                # outputs['sence'] = inputs['sence']
                outputs['coords'] = pre_coords
                outputs['tsdf'] = pre_tsdf
                outputs['tsdf_target'] = tsdf_target
                # outputs['coords'] = up_coords
        return outputs, loss_dict
    ...
    def compute_loss(tsdf, occ, tsdf_target, occ_target, loss_weight=(1, 1),
        ...
        # print("2",torch.sum(tsdf),torch.sum(occ),torch.sum(occ_target))
        ...
        # occ_loss = F.mse_loss(occ, occ_target.float())
