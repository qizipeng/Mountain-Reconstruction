...
from .modules import ConvGRU,transfusion
import numpy as np
...
class GRUFusion(nn.Module):
    ...
    def __init__(self, cfg, ch_in=None, direct_substitute=False, IsNeuralRecon = False):
        N_POINTS = [24, 32, 96]
        ...
        self.NeuralRecon = IsNeuralRecon
        ...
        self.hidden_dim = [48, 24, 12]
        # self.n_points =
        if direct_substitute:
            self.fusion_nets = None
            self.transfusion = None
        else:
            if not self.NeuralRecon:
                self.transfusion = nn.ModuleList()
                for i, ch in enumerate(ch_in):
                    ##self.index = self.getindex(N_POINTS[i],self.hidden_dim[i])
                    self.transfusion.append(transfusion(ch_in=ch, hidden_dim=ch,
                                                        input_dim=ch,
                                                        pres=1,
                                                        vres=1))  # self.cfg.VOXEL_SIZE * 2 ** (self.n_scales - i)))
            elif self.NeuralRecon:
    ...
    def save_mesh(self, scale, outputs, scene):
        print("savemesh in testing")
        if outputs is None:
            outputs = dict()
        if "scene_name" not in outputs:
            outputs['origin'] = []
            outputs['origin_target'] = []
            outputs['scene_tsdf'] = []
            outputs['scene_tsdf_target'] = []
            outputs['scene_name'] = []
        # if epoch_start:
        #     outputs['origin'] = []
        #     outputs['origin_target'] = []
        ...
        if scene in outputs['scene_name']:
            ...
            del outputs['origin_target'][idx]
            del outputs['scene_tsdf'][idx]
            del outputs['scene_tsdf_target'][idx]
        ...
        # print("tsdf.sahpe",tsdf.shape)
        tsdf_volume = sparse_to_dense_torch(ind_coords, tsdf, dim_list, 1, tsdf.device)
        outputs['scene_tsdf'].append(tsdf_volume)

        fuse_coords_target = self.target_tsdf_volume[scale].C
        tsdf_target = self.target_tsdf_volume[scale].F.squeeze(-1)
        # max_c = torch.max(fuse_coords_target, dim=0)[0][:3]
        # min_c = torch.min(fuse_coords_target, dim=0)[0][:3]
        # max_c =torch.Tensor([3.0,3.0,3.0]).cuda()
        # min_c =torch.Tensor([0.0,0.0,0.0]).cuda()
        outputs['origin_target'].append(min_c * self.cfg.VOXEL_SIZE * (2 ** (self.cfg.N_LAYER - scale - 1)))

        ind_coords_target = fuse_coords_target - min_c
        dim_list = (max_c - min_c + 1).int().data.cpu().numpy().tolist()
        tsdf_volume_target = sparse_to_dense_torch(ind_coords_target, tsdf_target, dim_list, 1, tsdf.device)
        outputs['scene_tsdf_target'].append(tsdf_volume_target)
        return outputs

    def forward(self, coords, values_in, inputs, scale=2, outputs=None, save_mesh=False, testing = False):
        '''
        ...
        for i in range(batch_size):
            ...
            if target_volume is not None:
                ...
                # print("tsdf:",tsdf_target.sum())
                # a = tsdf_target==1
                # print("a",a.sum())
                tsdf_target = target_volume[updated_coords[:, 0], updated_coords[:, 1], updated_coords[:, 2]]
                #print("tsdf:", tsdf_target.sum())
                occ_target = tsdf_target.abs() < 2.0

                # print("occ_target",occ_target.sum(0))
            else:
            ...
            if not self.direct_substitude:
                ...
                values = self.transfusion[scale](h,x)
