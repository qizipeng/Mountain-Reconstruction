...
import transforms3d
import torch
from tools.tsdf_fusion.fusion import TSDFVolumeTorch, TSDFVolume
from tools.tsdf_fusion.fusion import meshwrite
import os
...
class ToTensor(object):
    ...
    def __call__(self, data):
        ...
        # data['normals'] = torch.Tensor(np.stack(data['normals']).transpose([0, 3, 1, 2]))
        # data['normals'] = torch.Tensor(np.stack(data['normals']))
...
class RandomTransformSpace(object):
    ...
    def __init__(self, voxel_dim, voxel_size, random_rotation=True, random_translation=True,
                 paddingXY=1.5, paddingZ=.25, origin=[0, 0, 0], max_epoch=999, max_depth=4):
        """
        Args:
    ...
    def meshwrite(self, filename, verts, faces, norms):
    ...
    def get_mesh(self, tsdf_vol, weight_vol, voxel_size, vol_origin):
        """Compute a mesh from the voxel volume using marching cubes.
        """
        verts, faces, norms, vals = measure.marching_cubes_lewiner(tsdf_vol)
        verts_ind = np.round(verts).astype(int)
        verts = verts * voxel_size + vol_origin.numpy()  # voxel grid coordinates to world coordinates
        ...
        if 'tsdf_list_full' in data.keys():
            ...
            for l, tsdf_s in enumerate(data['tsdf_list_full']):
                ...
                tsdf_vol = TSDFVolumeTorch(vol_dim_s, vol_origin_partial,
                                           voxel_size=self.voxel_size * 2 ** l, margin=5)
                for i in range(data['imgs'].shape[0]):
                ...
                # tsdf_vol_tmp, weight_vol_tmp = tsdf_vol.get_volume()
                # tsdf_vol_tmp = tsdf_vol_tmp.numpy()
                # verts_tmp, faces_tmp, norms_tmp = self.get_mesh(tsdf_vol_tmp, weight_vol_tmp,self.voxel_size* 2 ** l,data['vol_origin_partial'] )
                # self.meshwrite(os.path.join("./mesh", '{}_mesh_layer{}.ply'.format(str(data['id']),str(l))), verts_tmp, faces_tmp, norms_tmp)


                tsdf_vol, weight_vol = tsdf_vol.get_volume()


                # tsdf_vol = TSDFVolume(bnds,
                #                            voxel_size=self.voxel_size * 2 ** l, margin=5)
                # for i in range(data['imgs'].shape[0]):
                #     depth_im = data['depth'][i]
                #     color_im = data['imgs'][i]
                #     cam_intr = data['intrinsics'][i]
                #     cam_pose = data['extrinsics'][i]
                #     # print("name_id",data['name_id'][i])
                #     tsdf_vol.integrate(color_im.numpy(),depth_im.numpy(), cam_intr.numpy(), cam_pose.numpy(), obs_weight=1.)
                #
                #
                # print("Saving mesh to mesh{}.ply...".format(str(l)))
                #
                # verts_tmp, faces_tmp, norms_tmp, _ = tsdf_vol.get_mesh()
                # self.meshwrite(os.path.join("./mesh2", '{}_mesh_layer{}.ply'.format(str(data['id']),str(l))), verts_tmp, faces_tmp, norms_tmp)
                #
                #
                # tsdf_vol, weight_vol = tsdf_vol.get_volume()
